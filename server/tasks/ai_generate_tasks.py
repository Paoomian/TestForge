...
def extract_json_from_response(response: str) -> list:
    ...
    for pattern in patterns:
        ...
        if match:
            try:
                ...
                fixed = try_fix_truncated_json(match.group(1).strip())
                if fixed:
                    return fixed
    ...
    # 尝试从整个响应中提取
    fixed = try_fix_truncated_json(response)
    if fixed:
        return fixed

    # 如果仍然失败，保存原始响应用于调试
    raise ValueError(f"无法从 AI 响应中提取有效的 JSON 数组。响应前 500 字符: {response[:500]}")
...
def generate_test_cases(self, task_id: int, config_id: int):
    ...
    try:
        ...
        # 构建 Prompt
        update_progress(40, "正在构建 AI 提示词...")
        if task.skill_id:
        ...
        # 更新进度
        update_progress(50, "AI 正在思考中...")

        # 调用 AI 生成
        response = run_async(provider.chat(messages, task.model_name))

        # 更新进度
        update_progress(80, "正在解析 AI 返回结果...")

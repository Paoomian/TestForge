...
import {
  IconSearch, IconRefresh, IconPlus, IconMore, IconFile,
  IconDesktop, IconPlayArrow, IconEdit, IconCommon,
} from '@arco-design/web-vue/es/icon'
import { getUITestSuites, deleteUITestSuite, runUITestSuite } from '@/api/uiTestSuite'
import type { UITestSuite } from '@/api/uiTestSuite'
import UITestSuiteDrawer from './components/UITestSuiteDrawer.vue'

const router = useRouter()
const loading = ref(false)
const tableData = ref<UITestSuite[]>([])
...
const handleEdit = (record: UITestSuite) => {
  editingSuite.value = record
  drawerVisible.value = true
}

const handleDelete = async (record: UITestSuite) => {
  try {
...
const handleRun = async (record: UITestSuite) => {
  try {
    const res = await runUITestSuite(record.id)
    Message.success('执行任务已创建')
    // 跳转到任务详情
    router.push({ name: 'ui-batch-run-detail', params: { runId: (res as any).id } })
  } catch (e: any) {
